...
class CryptographicSecurityTests(unittest.TestCase):
    ...
    def test_entropy_source_quality(self):
        ...
        try:
            ...
            # Collect samples
            random_bytes = [
                get_enhanced_random_bytes(sample_size) for _ in range(samples)
            ]
            ...
            margin = 0.5
            self.assertLess(
                abs(std_dev - expected_std_dev),
                margin * expected_std_dev,
                f"Byte distribution not within expected randomness bounds: "
                f"std_dev={std_dev}, expected={expected_std_dev}",
            )
            log_test_success(
                f"Byte distribution within expected limits (std dev: {std_dev:.2f})"
            )
            ...
            for i in range(1, len(random_bytes)):
                ...
                xor_result = bytes(
                    a ^ b for a, b in zip(random_bytes[i - 1], random_bytes[i])
                )
            ...
            expected_correlation = (
                32 * 8 * 0.5
            )  # Half the bits should be different on average

            # Allow a 15% margin for statistical variation (increased for robustness)
            margin = 0.15
            self.assertLess(
                abs(avg_correlation - expected_correlation),
                margin * expected_correlation,
                f"Sequential correlation detected: avg_distance={avg_correlation}, expected={expected_correlation}",
            )
            log_test_success(
                f"No sequential correlation detected (average distance: {avg_correlation:.2f})"
            )
    ...
    def test_key_reuse_prevention(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                test_content = (
                    b"This is identical test content for encryption testing" * 10
                )  # Make content longer
                ...
                # N3/C1 fix: use encrypt_file() — no plaintext hash, single crypto path
                encryptor1.encrypt_file(
                    test_file1, encrypted_file1,
                    extra_metadata={"test_metadata": "file1"}
                )
                ...
                encryptor2.encrypt_file(
                    test_file2, encrypted_file2,
                    extra_metadata={"test_metadata": "file2"}
                )

                # Verify files were created
                self.assertTrue(
                    os.path.exists(encrypted_file1), "First encrypted file should exist"
                )
                self.assertTrue(
                    os.path.exists(encrypted_file2),
                    "Second encrypted file should exist",
                )
                ...
                with (
                    ...
                    self.assertGreater(
                        len(content1),
                        0,
                        "First encrypted file content should not be empty",
                    )
                    self.assertGreater(
                        len(content2),
                        0,
                        "Second encrypted file content should not be empty",
                    )
                    ...
                    log_test_success(
                        "Encrypted files are different despite identical content"
                    )
                    ...
                    log_test_success(
                        "Decryption successful, original content recovered for both files"
                    )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with redirect_stdout(stdout_capture):
                    ...
                    for idx, share_data in shares:
                        ...
                        share_formats.append(
                            base64.b64encode(share_data).decode("ascii")
                        )
                ...
                self.assertNotIn(
                    test_key_hex, log_output, "Secret key (hex) found in logs"
                )
                self.assertNotIn(
                    test_key_b64, log_output, "Secret key (base64) found in logs"
                )
                ...
                for share_format in share_formats:
                    self.assertNotIn(
                        share_format,
                        log_output,
                        f"Share data found in logs: {share_format[:10]}...",
                    )
                ...
                with open(test_file, "r") as f:
                    ...
                    self.assertNotIn(
                        secret_content, log_output, "Secret file content found in logs"
                    )
...
class SideChannelSecurityTests(unittest.TestCase):
    ...
    def test_timing_attack_resistance(self):
        ...
        try:
            ...
            log_test_step(
                f"Measuring times for {threshold} shares across multiple trials"
            )
            ...
            for i in range(num_trials):
                ...
                self.assertEqual(
                    reconstructed, test_secret, "Reconstructed secret is incorrect"
                )
            ...
            log_test_success(
                f"Limited time variations: max deviation {max_deviation:.6f}s vs average {avg_time:.6f}s"
            )
            log_test_end(test_name)
        except Exception as e:
            log_test_end(test_name, success=False)
    ...
    def test_memory_usage_patterns(self):
        ...
        try:
            ...
            for i in range(1, len(secret_sizes)):
                ...
                log_test_step(
                    f"Size ratio {size_ratio:.2f} vs memory ratio {increase_ratio:.2f} - OK"
                )
        ...
        try:
            ...
            try:
                ...
                if isinstance(test_list, list):
                    self.assertEqual(
                        len(test_list), 0, "List should be emptied if possible"
                    )
    ...
    def test_adversarial_shares(self):
        ...
        try:
            ...
            with self.assertRaises(
                ValueError, msg="Should fail with inconsistent indices"
            ):
            ...
            corrupted_shares = [
                ...
                (
                    self.valid_shares[2][0],
                    get_enhanced_random_bytes(32),
                ),  # Valid index but random data
            ]
            ...
            with self.assertRaises(
                ValueError, msg="Should fail with insufficient shares"
            ):
            ...
            another_shares = self.manager.generate_shares(
                another_secret, "another_label"
            )
    ...
    def test_malicious_metadata(self):
        ...
        try:
            ...
            with (
                ...
                dst.write(
                    (100 * 1024 * 1024).to_bytes(4, "big")
                )  # Claim 100MB metadata
            ...
            with (
                ...
                malicious_metadata = metadata.replace(
                    b'"version"', b'"version\x00\x07\x1f"'
                )
            ...
            with open(encrypted_file, "rb") as src, open(recursive_file, "wb") as dst:
                ...
                original_content = src.read()
                metadata_len = int.from_bytes(original_content[:4], "big")
                encryption_data = original_content[
                    4 + metadata_len :
            ...
            for desc, malicious_file in test_cases:
                ...
                with self.subTest(f"Testing {desc} metadata"):
                    try:
                        ...
                        decrypted_file = (
                            self.test_dir
                            / f"{desc.lower().replace(' ', '_')}_decrypted.txt"
                        )
                        ...
                        log_test_success(
                            f"Case {desc}: Decryption successful, content correct"
                        )
                        ...
                        is_acceptable_error = any(
                            err in error_message for err in acceptable_errors
                        )
